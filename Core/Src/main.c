...
uint8_t Send_Sensor_DataBuffer[ 16 * 2 * sizeof( uint16_t ) ];
...
int main(void)
{
  ...
  Send_Sensor_DataHandle = osMessageQueueNew (16, 2*sizeof(uint16_t), &Send_Sensor_Data_attributes);
  ...
  /* USER CODE BEGIN Start_Reciver_task */
	...
	for(;;)
	{
		...
		if (status == osOK) {
			...
			osMutexAcquire(USART_MUTEXHandle, osWaitForever);
			uint16_t len = (uint16_t)sprintf(buff, "Roll angle = %d\r\nPitch angle = %d\r\n" , recived_data[0], recived_data[1]);
			HAL_UART_Transmit(&huart3, (uint8_t*)buff, len, 10);
			osMutexRelease(USART_MUTEXHandle);
